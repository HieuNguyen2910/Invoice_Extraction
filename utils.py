...
class Progress:
	...
	def __per_sec(self):
		prev = time()
		if self.__start == 0 or prev <= self.__start:   # tránh chia 0 hoặc giá trị âm
			ps = 0
		else:
			ps = 1 / (prev - self.__start)

		if len(self.__recently) >= 100:
			self.__recently.pop(0)

		if ps <= 100:  # lọc giá trị bất thường
			self.__recently.append(ps)

		self.__start = prev
		return np.mean(self.__recently) if self.__recently else 0
